...
public class FichaVehiculoView extends JFrame {
	...
	private void initialize() {
		...
		JLabel LBTitulo = new JLabel("Ficha de vehÃ­culos");
...
//***************		
		...
		PNCentral.setLayout(new BoxLayout(PNCentral, BoxLayout.Y_AXIS));
			...
			JLabel LBMatricula = new JLabel("MatrÃ­cula");
	...
	private boolean comprobarInt(String miStr) {
		...
		if (!(miStr.matches("[0-9]*"))) {
			resultado=false;
			JOptionPane.showMessageDialog(frame, "El precio no es un nÃºmero vÃ¡lido. O no ha introducido todos los datos requeridos.");
		};
		return resultado;
	}
